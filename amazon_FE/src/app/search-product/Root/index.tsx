...
export const handelProduct = (props: any) => {
    let { name } = props.searchParams;
    let [products, setProducts] = useState([]);

    useEffect(() => {
        //call API
        axios.get(`http://localhost:8080/get-product?name=${name}`).then(value => {
            // console.log(value);
    ...
    return products.map((item: any) => {
        return (
            <Col key={item.key} lg={{ flex: "20%" }}>
                <Link href={`/product-detail?id=${item.id}`} >
                    <img src={item.image} style={{ width: "95% ", height: "150px" }} alt="..." />
